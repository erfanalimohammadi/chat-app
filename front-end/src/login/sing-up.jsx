...
export default function Register(){
    ...
    function handleSubmit(e) {
        ...
        if(user.phoneNumber.trim() !== '' && isChecked){
            navigate('/enterd/Register/OTPCheck')
        }else{
            ...
            <div class="relative min-h-screen bg-white">
                ...
                <div class="relative z-10 p-8">
                    <div className='flex flex-row w-full px-2 justify-between items-center'>
                        <Link to={"/enterd/login"} className='rounded-[28px] flex flex-row justify-between items-center gap-x-3 bg-slate-200 text-black text-[20px] py-2 px-4 translate-y-1'>
                            <HiArrowSmallLeft/> login
                        </Link>
                        <h2 className='text-[35px] font-bold text-white'>Register</h2>
                    </div>
                    <div>
